    ...
    public class LineGraphDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            using (new NewIndentLevel(0)) //Draw the graph
            {
                ...
                List<Vector2> points = pointsProperty.GetAllProperties(false).Select<SerializedProperty, Vector2>(x => x.vector2Value).ToList();
                ...
                {
                    //sort changed values
                    var sortedPoints = pointsProperty.GetAllProperties(false).Select(x => x.vector2Value).OrderBy(p => p.x).ToList();
